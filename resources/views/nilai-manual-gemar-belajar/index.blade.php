...
<head>
    ...
    <title>Nilai Manual - Gemar Belajar - 7 Kebiasaan Anak Indonesia Hebat</title>
    <style>
        @keyframes fadeIn {
...
<body class="bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-7xl">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    <h1 class="text-2xl lg:text-3xl font-bold bg-gradient-to-r from-primary-600 to-accent-green bg-clip-text text-transparent mb-2">
                        Nilai Manual - Gemar Belajar
                    </h1>
                    <p class="text-gray-500 text-sm">
                        Manajemen penilaian kegiatan membaca dan literasi
                    </p>
                </div>
                <div class="flex items-center gap-3">
                    <div class="text-center">
                        <div class="text-xs text-gray-500">Total Data</div>
                        <div class="text-xl font-bold text-gray-900">{{ isset($data->total) ? $data->total() : ($totalData ?? $data->count()) }}</div>
                    </div>
                    <div class="h-8 w-px bg-gray-300"></div>
                    <div class="text-center">
                        <div class="text-xs text-gray-500">Belum Dinilai</div>
                        <div class="text-xl font-bold text-gray-900">{{ isset($pendingCount) ? $pendingCount : $data->where('status', 'pending')->count() }}</div>
                    </div>
                    <div class="h-8 w-px bg-gray-300"></div>
                    <div class="text-center">
                        <div class="text-xs text-gray-500">Sudah Dinilai</div>
                        <div class="text-xl font-bold text-gray-900">{{ isset($approvedCount) ? $approvedCount : $data->where('status', 'approved')->count() }}</div>
                    </div>
                </div>
            </div>
        ...
        <div class="glass-card rounded-2xl shadow-lg overflow-hidden fade-in" style="animation-delay: 0.3s;">
            ...
            <div class="px-6 py-5 border-b border-gray-100">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900">Daftar Gemar Belajar</h2>
                            <p class="text-gray-500 text-sm">
                                @if(isset($data->total))
                                    Total {{ $data->total() }} data ditemukan
                                @else
                                    Total {{ $data->count() }} data ditemukan
                                @endif
                            </p>
                        </div>
                    </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gradient-to-r from-primary-500/10 to-accent-green/10">
                            ...
                            <th class="p-4 text-left font-semibold text-gray-700 text-sm uppercase tracking-wider">Buku & Informasi</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="table-row-hover transition-all duration-200 {{ !$item->nilai ? 'bg-yellow-50/30' : '' }}">
                            ...
                            <td class="p-4">
                                <div class="space-y-1">
                                    <p class="text-gray-900 text-sm font-medium">{{ Str::limit($item->judul_buku, 40) }}</p>
                                    <p class="text-xs text-gray-500 line-clamp-2">
                                        {{ Str::limit($item->informasi_didapat, 80) }}
                                    </p>
                                </div>
                            </td>
                            <td class="p-4">
                            ...
                            <td class="p-4">
                                <div class="flex gap-2">
                                    @if($item->gambar_buku)
                                    <a href="{{ Storage::url($item->gambar_buku) }}" target="_blank" class="relative group" title="Gambar Buku">
                                        <img src="{{ Storage::url($item->gambar_buku) }}" 
                                    ...
                                    @else
                                    <div class="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400" title="Tidak ada gambar buku">
                                    ...
                                    @if($item->gambar_baca)
                                    <a href="{{ Storage::url($item->gambar_baca) }}" target="_blank" class="relative group" title="Gambar Membaca">
                                        <img src="{{ Storage::url($item->gambar_baca) }}" 
                                             class="w-8 h-8 object-cover rounded-lg border border-gray-200 hover:scale-110 transition-transform duration-200">
                                        <div class="absolute inset-0 bg-primary-500/10 opacity-0 group-hover:opacity-100 rounded-lg transition-opacity duration-200"></div>
                                    </a>
                                    @else
                                    <div class="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400" title="Tidak ada gambar membaca">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                        </svg>
                                    </div>
                            ...
                            <td class="p-4">
                                ...
                                @else
                                <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold bg-yellow-100 text-yellow-800 animate-pulse">
                                    <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <td class="p-4">
                                <div class="flex items-center space-x-2">
                                    <button type="button" 
                                        onclick="openModal({{ $item->id }}, '{{ $item->siswa ? addslashes($item->siswa->nama_lengkap ?: $item->siswa->nama) : 'Siswa' }}', '{{ addslashes($item->judul_buku) }}', '{{ $item->nilai ?? '' }}', '{{ addslashes($item->catatan_admin ?? '') }}')"
                                    ...
                                    <form action="{{ route('nilai-manual-gemar-belajar.reset', $item->id) }}" method="POST" class="inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" 
                        ...
                        <tr>
                            <td colspan="8" class="p-8 text-center">
                                ...
                                <p class="text-gray-500 text-sm">Belum ada data gemar belajar yang perlu dinilai.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="lg:hidden p-4 space-y-4">
                @forelse ($data as $item)
                <div class="glass-card rounded-xl border border-gray-200 p-4 fade-in {{ !$item->nilai ? 'bg-yellow-50/30' : '' }}">
                    <div class="flex items-start mb-4">
                        <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-primary-500 to-accent-green flex items-center justify-center text-white font-bold text-lg mr-3">
                            {{ strtoupper(substr($item->siswa->nama_lengkap ?: $item->siswa->nama ?? 'S', 0, 1)) }}
                        </div>
                        <div class="flex-1">
                            ...
                            <p class="text-gray-900 text-sm font-medium">{{ Str::limit($item->judul_buku, 30) }}</p>
                    ...
                    @if($item->informasi_didapat)
                    <div class="mb-3">
                        <p class="text-xs text-gray-500 mb-1">Informasi:</p>
                        <p class="text-xs text-gray-600 line-clamp-2">{{ Str::limit($item->informasi_didapat, 80) }}</p>
                    </div>
                    @endif
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex gap-2">
                            @if($item->gambar_buku)
                            <a href="{{ Storage::url($item->gambar_buku) }}" target="_blank" class="relative" title="Gambar Buku">
                                <img src="{{ Storage::url($item->gambar_buku) }}" 
                                     class="w-8 h-8 object-cover rounded-lg border border-gray-200">
                            </a>
                            @endif
                            @if($item->gambar_baca)
                            <a href="{{ Storage::url($item->gambar_baca) }}" target="_blank" class="relative" title="Gambar Membaca">
                                <img src="{{ Storage::url($item->gambar_baca) }}" 
                                     class="w-8 h-8 object-cover rounded-lg border border-gray-200">
                            </a>
                            @endif
                        ...
                        <span class="inline-flex px-2 py-1 rounded-lg text-xs font-semibold 
                            {{ $item->status == 'approved' ? 'bg-emerald-100 text-emerald-800' : 'bg-yellow-100 text-yellow-800' }}">
                            {{ $item->status == 'approved' ? 'Dinilai' : 'Pending' }}
                        </span>
                        
                        @if($item->nilai)
                        <div class="bg-gradient-to-br from-primary-500 to-accent-green text-white px-3 py-1 rounded-lg">
                            <div class="text-sm font-bold">{{ $item->nilai }}</div>
                            <div class="text-xs text-primary-100">/100</div>
                        </div>
                        @else
                        <div class="bg-gray-100 text-gray-400 px-3 py-1 rounded-lg">
                    ...
                    <div class="flex gap-2 pt-3 border-t border-gray-100">
                        <button type="button" 
                            onclick="openModal({{ $item->id }}, '{{ $item->siswa ? addslashes($item->siswa->nama_lengkap ?: $item->siswa->nama) : 'Siswa' }}', '{{ addslashes($item->judul_buku) }}', '{{ $item->nilai ?? '' }}', '{{ addslashes($item->catatan_admin ?? '') }}')"
                            class="flex-1 {{ !$item->nilai ? 'bg-gradient-to-r from-accent-green to-emerald-600 text-white' : 'bg-gradient-to-r from-primary-500 to-primary-600 text-white' }} 
                                font-medium py-2.5 rounded-lg text-sm flex items-center justify-center gap-1">
                            @if(!$item->nilai)
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>
                        ...
                        @if($item->nilai)
                        <form action="{{ route('nilai-manual-gemar-belajar.reset', $item->id) }}" method="POST" class="flex-1">
                            @csrf @method('DELETE')
                            <button type="submit" 
                                    onclick="return confirm('Yakin ingin mereset nilai untuk {{ $item->siswa->nama_lengkap ?: $item->siswa->nama }}?')" 
                                    class="w-full bg-red-500 text-white font-medium py-2.5 rounded-lg hover:bg-red-600 transition-colors text-sm flex items-center justify-center gap-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                </svg>
                                Reset
                ...
                @empty
                <div class="glass-card rounded-xl border border-gray-200 p-8 text-center">
                    <div class="w-16 h-16 bg-gradient-to-br from-primary-50 to-blue-100 rounded-full flex items-center justify-center mx-auto mb-4">
                        <svg class="w-8 h-8 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                        </svg>
                ...
                <form id="nilaiForm" method="POST" action="">
                    ...
                    <div class="px-6 py-5">
                        ...
                        <div class="mb-4 p-3 bg-gradient-to-r from-accent-green/10 to-emerald-50 rounded-lg">
                            ...
                            <p class="text-sm font-medium text-gray-800" id="modalJudulBuku">-</p>
                        </div>
    ...
    <script>
        ...
        const baseUrl = '{{ route("nilai-manual-gemar-belajar.simpan", ":id") }}';

        // Open Modal
        function openModal(id, siswaName, judulBuku, currentNilai, catatanAdmin = '') {
