...
<head>
    ...
    <title>Buat Pengaturan Waktu Tidur Cepat - 7 Kebiasaan Anak Indonesia Hebat</title>
    <style>
        @keyframes fadeIn {
...
<body class="bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold gradient-primary-text">
                            Buat Pengaturan Waktu Tidur Baru
                        </h1>
        ...
        <div class="glass-card rounded-2xl shadow-lg overflow-hidden fade-in" style="animation-delay: 0.1s;">
            <div class="px-8 py-6 border-b border-gray-100">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Form Data Pengaturan Baru</h2>
                        <p class="text-gray-500 text-sm">Buat pengaturan dengan kategori tetap</p>
                    </div>
                </div>
            </div>

            <form action="{{ route('pengaturan-waktu-tidur-cepat.store') }}" method="POST" class="p-8" id="settingForm">
                @csrf
                ...
                <div class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input 
                                    ...
                                    value="{{ old('nama_pengaturan', 'Pengaturan Waktu Tidur') }}"
                                    placeholder="Contoh: Pengaturan Waktu Tidur SD"
                                >
                                <p class="text-xs text-gray-500 mt-2">Nama pengaturan akan dikunci setelah disimpan</p>
                            </div>
                    ...
                    <div class="fade-in" style="animation-delay: 0.25s;">
                        ...
                        <div id="kategoriContainer" class="space-y-4">
                            <!-- Kategori akan ditambahkan di sini secara dinamis -->
                        </div>

                        <!-- Template Kategori (Hidden) -->
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            required 
                                            readonly
                                            class="kategori-nama-input disabled-input w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center space-x-2">
                                            <input 
                                                ...
                                                class="w-10 h-10 cursor-pointer rounded-lg border border-gray-300"
                                                value="#10B981"
                                            >
                                            <span class="color-preview"></span>
                                            <input 
                                                type="text" 
                                                class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
                                                placeholder="#10B981"
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            min="1"
                                            required 
                                            class="urutan-input w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
                                            value="1"
                                            readonly
                                        >
                                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Tambahkan kategori default jika tidak ada error
            @if(!$errors->any())
            tambahKategori(); // Kategori 2
            tambahKategori(); // Kategori 3
            @endif

            // Update urutan saat halaman dimuat
            updateUrutan();
        });
        ...
        function tambahKategori() {
            ...
            const namaKategori = kategoriNames[index] || `Kategori ${index + 1}`;
            ...
            inputs.forEach(input => {
                ...
                // Update waktu untuk kategori selanjutnya
                if (input.name && input.name.includes('waktu_start') && index > 0) {
                    const prevKategori = container.querySelector(`.kategori-item:last-child input[name*="waktu_start"]`);
                    if (prevKategori) {
                        const prevTime = prevKategori.value;
                        const prevHour = parseInt(prevTime.split(':')[0]);
                        const newHour = (prevHour + 1) % 24;
                        input.value = newHour.toString().padStart(2, '0') + ':00';
                    }
                }
                
                if (input.name && input.name.includes('waktu_end') && index > 0) {
                    const prevKategori = container.querySelector(`.kategori-item:last-child input[name*="waktu_start"]`);
                    if (prevKategori) {
                        const prevTime = prevKategori.value;
                        const prevHour = parseInt(prevTime.split(':')[0]);
                        const newHour = (prevHour + 2) % 24;
                        input.value = newHour.toString().padStart(2, '0') + ':00';
                    }
                }
                
                if (input.name && input.name.includes('nilai') && index > 0) {
                    const prevKategori = container.querySelector(`.kategori-item:last-child input[name*="nilai"]`);
                    if (prevKategori) {
                        const prevNilai = parseInt(prevKategori.value);
                        input.value = Math.max(0, prevNilai - 25);
                    }
                }
                
                if (input.name && input.name.includes('warna') && index > 0) {
                    const colors = ['#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#6B7280'];
                    input.value = colors[index] || '#6B7280';
                }
            });
            
            // Update nama kategori di header
            const namaInput = kategoriDiv.querySelector('.kategori-nama-input');
            const namaHeader = kategoriDiv.querySelector('.kategori-nama');
            namaHeader.textContent = namaKategori;
            
            // Update color preview
            const colorPicker = kategoriDiv.querySelector('input[type="color"]');
            const colorText = kategoriDiv.querySelector('input[type="text"][placeholder="#10B981"]');
        ...
        document.getElementById('settingForm').addEventListener('submit', function(e) {
            ...
            // Validasi nama pengaturan unik (dapat ditambahkan validasi AJAX di sini)
            const namaPengaturan = document.querySelector('input[name="nama_pengaturan"]').value;
            if (!namaPengaturan.trim()) {
                e.preventDefault();
                alert('Nama pengaturan harus diisi');
                return;
            }
            ...
            kategoriItems.forEach((item, index) => {
                const start = item.querySelector('.waktu-start').value;
                const end = item.querySelector('.waktu-end').value;
            ...
            for (let i = 0; i < ranges.length; i++) {
                for (let j = i + 1; j < ranges.length; j++) {
                    if (checkTimeOverlap(ranges[i], ranges[j])) {
                        alert(`Kategori ${i + 1} dan ${j + 1} memiliki waktu yang tumpang tindih`);
                        isValid = false;
                        break;
                    }
                }
