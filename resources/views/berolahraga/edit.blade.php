...
<head>
    ...
    <title>Edit Data Olahraga - 7 Kebiasaan Anak Indonesia Hebat</title>
    <style>
        @keyframes fadeIn {
...
<body class="bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
        ...
        <div class="glass-card rounded-2xl shadow-lg overflow-hidden fade-in" style="animation-delay: 0.25s;">
            <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-primary-50 to-blue-50">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Form Edit Olahraga</h2>
                        <p class="text-gray-500 text-sm">Data ID: #{{ $berolahraga->id }}</p>
                    </div>
                </div>
            </div>

            <form action="{{ route('berolahraga.update', $berolahraga->id) }}" method="POST" enctype="multipart/form-data" id="formOlahraga">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 md:p-8 space-y-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="date" 
                                       id="tanggal" 
                                       name="tanggal" 
                                       value="{{ old('tanggal', $berolahraga->tanggal) }}"
                                       max="{{ date('Y-m-d') }}"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="time" 
                                       id="waktu_mulai" 
                                       name="waktu_mulai" 
                                       value="{{ old('waktu_mulai', $berolahraga->waktu_mulai) }}"
                                       class="w-full px-4 py-3 pl-10 border border-gray-200 rounded-xl focus:border-primary-500 transition-all duration-300 text-gray-900 bg-white shadow-sm"
                                       required>
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="time" 
                                       id="waktu_selesai" 
                                       name="waktu_selesai" 
                                       value="{{ old('waktu_selesai', $berolahraga->waktu_selesai) }}"
                                       class="w-full px-4 py-3 pl-10 border border-gray-200 rounded-xl focus:border-primary-500 transition-all duration-300 text-gray-900 bg-white shadow-sm"
                                       required>
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            ...
                            Video Bukti Olahraga
                            <span class="text-xs text-gray-500">(Kosongkan jika tidak ingin mengubah video)</span>
                        </label>
                        ...
                        <div class="mb-4 video-preview-container rounded-2xl p-6">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="font-medium text-gray-900">Video saat ini</p>
                                        <p class="text-sm text-gray-500">Uploaded on {{ $berolahraga->created_at->format('d/m/Y H:i') }}</p>
                                        <a href="{{ Storage::url($berolahraga->video_path) }}" 
                                           target="_blank"
                                           class="text-primary-600 hover:text-primary-800 text-sm inline-flex items-center gap-1 mt-1">
                            ...
                            <div class="upload-area border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center transition-colors duration-300 bg-gray-50" 
                                ...
                                <div id="uploadContent">
                                    ...
                                    <p class="text-lg font-medium text-gray-600 mb-2">
                                        <span class="text-primary-600 hover:text-primary-700 cursor-pointer">Klik di sini</span> untuk upload video baru
                                    </p>
                                    <p class="text-sm text-gray-500">Format: MP4, MOV, AVI, WMV (Maksimal 5MB)</p>
                                    @if($berolahraga->video_path)
                                        <p class="text-sm text-yellow-600 mt-2">⚠️ Upload video baru akan menggantikan video saat ini</p>
                                    @endif
                                </div>
                        ...
                        <div class="mt-4">
                            <label class="flex items-center">
                                ...
                                <span class="ml-2 text-sm text-gray-700">Hapus video saat ini tanpa mengganti yang baru</span>
                            </label>
                        </div>
                        @endif
    ...
    function confirmVideoDelete() {
        ...
        if (confirm('Apakah Anda yakin ingin menghapus video saat ini?')) {
            hapusVideoCheckbox.checked = true;
            // Hapus video baru jika ada
            videoInput.value = '';
            removeVideoFile();
        }
    }
    ...
    videoInput.addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            // Uncheck hapus video jika upload file baru
            const hapusVideoCheckbox = document.getElementById('hapus_video');
            if (hapusVideoCheckbox) {
                hapusVideoCheckbox.checked = false;
            }
    ...
    document.getElementById('formOlahraga').addEventListener('submit', function(e) {
        ...
        // Validasi video baru jika diupload
        ...
        if (videoFile) {
            ...
            // Jika ada video baru dan checkbox hapus dicentang, beri peringatan
            if (hapusVideoCheckbox && hapusVideoCheckbox.checked) {
                if (!confirm('Anda memilih menghapus video saat ini, tetapi juga mengupload video baru. Video baru akan digunakan. Lanjutkan?')) {
                    e.preventDefault();
                    return;
                }
            }
