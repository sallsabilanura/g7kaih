...
<head>
    ...
    <title>Edit Gemar Belajar - 7 Kebiasaan Anak Indonesia Hebat</title>
    <style>
        @keyframes fadeIn {
...
<body class="bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-2xl">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 text-sm mt-1">
                            Edit catatan membaca siswa
                        </p>
                    </div>
                </div>
                <div class="bg-gradient-to-r from-primary-50 to-accent-green/10 px-4 py-2 rounded-lg">
                ...
                <div class="flex items-center space-x-3">
                    <div class="w-12 h-12 rounded-full bg-gradient-to-r from-primary-500 to-accent-green flex items-center justify-center">
                        <span class="text-white font-bold text-lg">
                            {{ strtoupper(substr($gemarBelajar->siswa->nama_lengkap, 0, 1)) }}
                        </span>
                    </div>
                    <div>
                        <h4 class="font-semibold text-gray-900">{{ $gemarBelajar->siswa->nama_lengkap }}</h4>
                        <div class="flex flex-col md:flex-row md:items-center space-y-1 md:space-y-0 md:space-x-3 text-sm text-gray-600">
                            <span>NIS: {{ $gemarBelajar->siswa->nis }}</span>
                            @if($gemarBelajar->siswa->kelas)
                                <span class="hidden md:inline text-gray-300">•</span>
                                <span>Kelas: {{ $gemarBelajar->siswa->kelas->nama_kelas ?? '-' }}</span>
                            @endif
                            <span class="hidden md:inline text-gray-300">•</span>
                            <span>Ditambahkan: {{ \Carbon\Carbon::parse($gemarBelajar->created_at)->format('d M Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="glass-card rounded-2xl shadow-lg overflow-hidden fade-in" style="animation-delay: 0.1s;">
            <div class="px-8 py-6 border-b border-gray-100">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Form Edit Data Gemar Belajar</h2>
                        <p class="text-gray-500 text-sm">Update data catatan membaca siswa</p>
                    </div>
                </div>
            </div>

            <form action="{{ route('gemar-belajar.update', $gemarBelajar->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="fade-in" style="animation-delay: 0.2s;">
                        ...
                        <div class="date-display">
                            ...
                            <span id="current-date">{{ \Carbon\Carbon::parse($gemarBelajar->tanggal)->translatedFormat('d F Y') }}</span>
                        </div>
                        <input 
                            type="hidden" 
                            name="tanggal" 
                            value="{{ $gemarBelajar->tanggal }}"
                        >
                        @error('tanggal')
                        ...
                        @enderror
                        <p class="text-xs text-gray-500 mt-2">Tanggal catatan tidak dapat diubah</p>
                    </div>
                    ...
                    <div class="fade-in" style="animation-delay: 0.25s;">
                        ...
                        <input 
                            ...
                            value="{{ old('judul_buku', $gemarBelajar->judul_buku) }}"
                        >
                        @error('judul_buku')
                            <p class="text-red-500 text-sm mt-2 flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="fade-in" style="animation-delay: 0.3s;">
                        ...
                        >{{ old('informasi_didapat', $gemarBelajar->informasi_didapat) }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="image-info">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm">Gambar buku tersimpan</span>
                                </div>
                                <div class="image-actions">
                                    <a href="{{ asset('storage/' . $gemarBelajar->gambar_buku) }}" 
                                       target="_blank"
                                       class="image-action-btn view-btn">
                                        Lihat
                                    </a>
                                    <button type="button" 
                                            onclick="toggleUpload('buku')"
                                            class="image-action-btn change-btn">
                                        Ganti
                                    </button>
                                </div>
                            </div>
                            
                            <!-- Checkbox untuk menjaga gambar lama -->
                            <div class="keep-current-label">
                                <input type="checkbox" 
                                       id="keep_gambar_buku" 
                                       name="keep_gambar_buku" 
                                       value="1" 
                                       checked
                                       class="rounded border-gray-300 text-primary-600 focus:ring-primary-500">
                                <label for="keep_gambar_buku">Gunakan gambar yang sudah ada</label>
                            </div>
                            @endif
                            
                            <!-- Input File (Sembunyikan jika ada gambar) -->
                            <div class="relative @if($gemarBelajar->gambar_buku) hidden @endif" id="uploadBuku">
                                <input 
                                    type="file" 
                                    name="gambar_buku"
                                    id="gambar_buku"
                            ...
                            <!-- Preview untuk Gambar Buku Baru -->
                            <div class="image-preview-container mt-3 @if($gemarBelajar->gambar_buku) current-image @endif" 
                                 id="previewContainerBuku" 
                                 style="display: @if($gemarBelajar->gambar_buku) block @else none @endif;">
                                <div class="image-preview" id="previewBuku">
                                    @if($gemarBelajar->gambar_buku)
                                        <img src="{{ asset('storage/' . $gemarBelajar->gambar_buku) }}" alt="Gambar Buku Saat Ini">
                                    @endif
                                </div>
                        ...
                        <div>
                            ...
                            <div class="image-info">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm">Gambar membaca tersimpan</span>
                                </div>
                                <div class="image-actions">
                                    <a href="{{ asset('storage/' . $gemarBelajar->gambar_baca) }}" 
                                       target="_blank"
                                       class="image-action-btn view-btn">
                                        Lihat
                                    </a>
                                    <button type="button" 
                                            onclick="toggleUpload('baca')"
                                            class="image-action-btn change-btn">
                                        Ganti
                                    </button>
                                </div>
                            ...
                            <div class="keep-current-label">
                                <input type="checkbox" 
                                       id="keep_gambar_baca" 
                                       name="keep_gambar_baca" 
                                       value="1" 
                                       checked
                                       class="rounded border-gray-300 text-primary-600 focus:ring-primary-500">
                                <label for="keep_gambar_baca">Gunakan gambar yang sudah ada</label>
                            </div>
                            @endif
                            
                            <!-- Input File (Sembunyikan jika ada gambar) -->
                            <div class="relative @if($gemarBelajar->gambar_baca) hidden @endif" id="uploadBaca">
                                <input 
                                    type="file" 
                                    name="gambar_baca"
                                    id="gambar_baca"
                            ...
                            <!-- Preview untuk Gambar Baca Baru -->
                            <div class="image-preview-container mt-3 @if($gemarBelajar->gambar_baca) current-image @endif" 
                                 id="previewContainerBaca" 
                                 style="display: @if($gemarBelajar->gambar_baca) block @else none @endif;">
                                <div class="image-preview" id="previewBaca">
                                    @if($gemarBelajar->gambar_baca)
                                        <img src="{{ asset('storage/' . $gemarBelajar->gambar_baca) }}" alt="Gambar Membaca Saat Ini">
                                    @endif
                                </div>
                                <div class="preview-overlay">
                                    <div class="checkmark">
                ...
                <div class="flex flex-col sm:flex-row gap-4 mt-12 pt-8 border-t border-gray-100 fade-in" style="animation-delay: 0.4s;">
                    ...
                    <button 
                        ...
                        class="flex-1 bg-red-50 text-red-700 font-semibold py-3.5 rounded-xl shadow-sm hover:shadow-md transform hover:-translate-y-0.5 transition-all duration-300 text-center group hover:bg-red-100 border border-red-200"
                    >
        ...
        <!-- Delete Form (Hidden) -->
        <form id="delete-form" action="{{ route('gemar-belajar.destroy', $gemarBelajar->id) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    ...
    <script>
        ...
        function previewImage(input, previewId) {
            ...
            const type = previewId === 'previewBuku' ? 'buku' : 'baca';
            
            const preview = document.getElementById(previewId);
            const previewContainer = document.getElementById(previewContainerId);
            const fileInfo = document.getElementById(fileInfoId);
            const keepCheckbox = document.getElementById(`keep_gambar_${type}`);
            const fileInput = input;
            
            // Uncheck "keep current" checkbox jika user upload file baru
            if (keepCheckbox) {
            ...
            if (fileInput.files && fileInput.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    previewContainer.classList.remove('current-image');
        ...
        // Fungsi untuk toggle upload field
        function toggleUpload(type) {
            const uploadDiv = document.getElementById(`upload${type.charAt(0).toUpperCase() + type.slice(1)}`);
            const keepCheckbox = document.getElementById(`keep_gambar_${type}`);
